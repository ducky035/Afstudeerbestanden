...
namespace Encog.Examples.SVMPredictCSV
{
    ...
    public class MarketPredict
    {
        ...
        {
            BasicMLDataSet set = Analyzer.CreateEval.CreateEvaluationSetAndLoad(app.Args[1], CONFIG.STARTING_YEAR,
                                                                                              CONFIG.TRAIN_END,
                                                                                              CONFIG.INPUT_WINDOW,
            ...
            BasicNetwork network =
                (BasicNetwork)Analyzer.CreateEval.CreateElmanNetwork(CONFIG.INPUT_WINDOW, CONFIG.PREDICT_WINDOW);

            //Train it..

            double LastError = Analyzer.CreateEval.TrainNetworks(network, set);
        ...
        {
            //BasicMLDataSet set = Analyzer.CreateEval.Analyzer.CreateEvaluationSetAndLoad(app.Args[1],1000,500,CONFIG.INPUT_WINDOW,CONFIG.PREDICT_WINDOW);


            TemporalMLDataSet Tempo = Analyzer.CreateEval.GenerateATemporalSet(app.Args[1], 1000, 500, CONFIG.INPUT_WINDOW, CONFIG.PREDICT_WINDOW);
